    ...
    while True:
        try:
            ...
            run = cliente.beta.threads.runs.create(
                thread_id=thread_id, assistant_id=assistente_id
            )
                ...
                if run.status == STATUS_REQUIRES_ACTION:
                    ...
                    for uma_tool in tools_acionadas:
                        ...
                        respostas_tools_acionadas.append(
                            {"tool_call_id": uma_tool.id, "output": resposta_funcao}
                        )
            ...
            historico = list(
                cliente.beta.threads.messages.list(thread_id=thread_id).data
            )
